...
(defn deg2rad [degrees]
  ...
  ;;;;;Notes
;This is fairly specialized.  Only use this code for your base if you are planning on making a RentalCar (ie 3 Row dactyl).  
; This has my tweaks
...
;(def case-text "3D ortho by")
(def case-text "")
(def case-text2 "")
;(def case-text2 "/u/crystalhand")
...
;;;;;;Alpha area curve;;;;;;;;;;		
;@@@@@@@@@@@@@@@@@@@@@@@@@@
(def front-back-curve (deg2rad 23))					;;Default is 15 front to back curve of alpha area.
...
(def additional-thumb-x-offset -5)  				;;additional offset for thumb cluster-0 by default
(def thumb_y_offset 3)							;;additional offset for thumb cluster y direction.  needed for 3_cylinder
...
(def front-back-thumb-tilt 15)   		 		;default 15 for better bottom fit originally 15
...
(def z-thumb-rotation (deg2rad 1.5))			;;default of 3.5
...
(def wrist-rest-rotation-angle 9)				;;Default 9 The angle in counter clockwise the wrist rest is at			
...
(def nrows (if (= row-size 0) (- 6 row-size) (if (= row-size 1) (-7 row-size) (- 8 row-size))))
...
;; Switch Hole ;;
;;;;;;;;;;;;;;;;;
(def keyswitch-height 14.55) ;; Was 14.1, then 14.25		
...
(def box-single-plate
  (let [top-wall (->> (cube (+ keyswitch-width 3) 1.5 plate-thickness)
       ...
       side-nub (->> (binding [*fn* 30] (cube 0.7 0.85 8.75));last number is nub size.  4.75 works for box
                      (rotate (/ π 2) [1 0 0])
                      (translate [(+ (/ keyswitch-width 2)) 0 (- plate-thickness 0.8)]) ;last number control nub height
...
(def kalih_socket
	(difference
		(union (cube 11 6.1 1.9) 
			(->> (union
				(->> (cylinder 1.6 1.5)(with-fn 100)(translate [0 0 0]))
				(->> (cylinder 1.6 1.5)(with-fn 100)(translate [6.35 2.540 0]))) (translate [-3.17 -1.27 1.65]))
				(->> (cube 2.7 1.9 1.8)(translate [6.8 1.5 0]))
				(->> (cube 2.7 1.9 1.8)(translate [-6.8 -1.5 0]))
			;	(->> (cube 3 3 3)(translate [2 -4.2 0]))
				)
		(->> (cube 3 3 3)(translate [4 -2.5 0]))
	)
)
...
(def test_switch
	(->>(difference 
		...
		(cube 14.3, 13.8, 4)
		(->>(cube 14.2, 15., 2.02)(translate [0, 0, -1]))
		(->> (cube 3.7,15.,  4)(translate [(/ 8.5 2), 0, 0]))
		(->> (cube 3.7,15.,  4)(translate [(/ -8.5 2), 0, 0]))
	)(translate [0, 0 2.25]))
...
(defn key-place [column row shape]
  (let [row-placed-shape (->> shape
                              ...
                              (rotate (* α (- centerrow row)) [1 0 0])	;;;;;;;;;;;;;;<--changed
                              (translate [0 0 row-radius]))
        column-offset (cond
...
(defn case-place [column row shape]
  (let [row-placed-shape (->> shape
                              ...
                              (translate [0 0 row-radius]))
        column-offset [0 -5.5 5.64]		;;Controls offset at bottom between case wall and first key  changedr
        column-angle (* β (- 2 column))
        placed-shape (->> row-placed-shape
                          (translate [0 0 (- column-radius)])
                          (rotate column-angle [0 1 0])
...
(defn thumb-place [column row shape]
  (let [cap-top-height (+ plate-thickness sa-profile-key-height)
    ...
    (->> shape
	  ...
	  (translate [(+ -52 additional-thumb-x-offset) (+ thumb_y_offset -45) (+ 40 top-z-offset-thumb)]))))
...
(if (= switch-type 2) (thumb-place 0 -1/2  (rotate (/ π 2) [0 0 1]shape))
   ...
   (let [thumb-tl #(->> web-post-tl
     ...
     (union
      ...
      (triangle-hulls (thumb-place 0 -1/2 (thumb-br 2))
                      ...
                      ;(key-place 1 3 web-post-bl)
                      (thumb-place 0 -1/2 (thumb-tr 2))
                      (key-place 0 4 web-post-br)
                      (key-place 0 4 web-post-bl)
                      (thumb-place 0 -1/2 (thumb-tr 2))
                      (thumb-place 0 -1/2 (thumb-tl 2))
                      (key-place 0 4 web-post-bl)
                   ;   (thumb-place 1.0 -0.34 (thumb-tr 1.55))
                      (thumb-place 1 7/8 (thumb-br 1.25));Fine tune this line
                      (key-place 0 3 web-post-bl)
                     (key-place 0 3 web-post-tl)
                      (thumb-place 1 7/8 (thumb-br 1.25))
...
(def thumbcaps-2u
  (union
   (thumb-2x-column-2u (rotate(/ π 2) [0 0 1] (sa-cap 2)))
   (thumb-place 1 -1/2 (sa-cap 2))
   (thumb-place 1 1 (sa-cap 1))
   ...
   (let [plate-height (/ (- sa-double-length mount-height) 2)
     ...
     (union
      ...
      (triangle-hulls (thumb-place 0 -1/2 thumb-br)
					 ...
					 (thumb-place 0 -1/2 thumb-tr)
					    (thumb-place 0 -1/2 thumb-tl)
                     (key-place 0 4  web-post-bl);;need to change this one. old  (thumb-place 0 -1/2 thumb-tr)
                     (thumb-place 0 -1/2 thumb-tl)
                      (key-place 0 3 web-post-bl) ;;this is the one at issue maybe closer (key-place 0 4 thumb-tl)
                      (thumb-place 1 -1/2 thumb-tr)
                      (thumb-place 1 1 web-post-br)
                     (key-place 0 3 web-post-bl)
                     (key-place 0 3 web-post-tl)
...
(def front-wall
  (let [step wall-step ;;0.1
	...
	(if (== sidewall-height 0.) () (union 		 
			(top-cover 0.5 1.7 3.7 4)
			(top-cover 1.59 2.41 3.52 4) ;; was 3.32@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
			(top-cover 2.39 3.41 3.7 4) ;;;changed increased 3rd number by .1) 
		 ))
	 (if (= alpha_ergo_style 1)
     ...
     (hull (place right-wall-column 4 (translate [0 1 1] wall-sphere-bottom-front))
           ...
           (key-place (- ncols 1) 4 web-post-bl)
           (key-place (- ncols 1) 4 web-post-br))
...
(def back-wall
  (let [back-row (first rows)
    ...
    (if (== sidewall-height 0) ()(union
		 ...
		 (front-top-cover 3.56 4.44 back-y (+ back-y 0.25));2nd from right overhand
		 (front-top-cover 4.3 right-wall-column back-y (+ back-y 0.25))	;;Edge most column
	))


     (hull (place left-wall-column back-y (translate [1 -1 1] wall-sphere-bottom-back))	;;left most side 
     ...
     (hull (place (- ncols 1) back-y (translate [0 -1 1] wall-sphere-bottom-back)) ;;This is the rightmost section of the back
           (place right-wall-column back-y (translate [0 -1 1] wall-sphere-bottom-back))
           (key-place (- ncols 1) back-row web-post-tl)
          (if (== alpha_ergo_style 1) (key-place (- ncols 1) back-row (translate[5 0 0] web-post-tr )))
		  (if (== alpha_ergo_style 0) (key-place (- ncols 1) back-row web-post-tr ))
		   )
   ...
   (hull (place (- 5 1/2) back-y (translate [0 -1 1] wall-sphere-bottom-back))
           (place (- ncols 1) back-y (translate [0 -1 1] wall-sphere-bottom-back))
             ...
             (for [x (range row-size 5)]
               (union
                (hull (place right-wall-column x (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (key-place (- ncols 1) x web-post-br)
                      (key-place (- ncols 1) x web-post-tr))))
             (for [x (range row-size 4)]
               (union
                (hull (place right-wall-column x (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (place right-wall-column (inc x) (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                      (key-place (- ncols 1) x web-post-br)
                      (key-place (- ncols 1) (inc x) web-post-tr))))
             [(union
               (hull (place right-wall-column (first rows) (translate [-1 0 1] (wall-sphere-bottom 1/2)))
                     ...
                     (key-place (- ncols 1) (first rows) web-post-tr))
...
;;the below 2 need to be changed for dactyl		   
    #_ (color [0 1 0] (hull (place left-wall-column row-size (translate [1 -1 1] wall-sphere-bottom-back))
     ...
     (if (== row-size 1) (color [0 1 0] (hull (place left-wall-column 1 (translate [1 -1 1] wall-sphere-bottom-back))
                          (place left-wall-column 2 (translate [1 0 1] wall-sphere-bottom-back))
...
(def left-wall-lightcycle			;;changed
	(let [place case-place]
                     [#_(hull (bottom-hull(place left-wall-column 0.8 (translate [1 -1 1]  bottom-wall-sphere-bottom-back)))
                           (bottom-hull (place left-wall-column 2 (translate [1 0 1]  bottom-wall-sphere-bottom-back)))
							)             
                      (hull (bottom-hull(place left-wall-column 1.85 (translate [1 0 1]  bottom-wall-sphere-bottom-back)))
                           (bottom-hull(place left-wall-column 1.6666  (translate [0.5 -1.5 1]  bottom-wall-sphere-bottom-front)))
							)
                      #_(hull (bottom-hull(place left-wall-column 1.6666  (translate [1 0 1]  bottom-wall-sphere-bottom-front)))
							)]))
					

 (def thumb-back-wall-lower [(hull
                        (bottom-hull   (thumb-place 0.5 thumb-back-y (translate [0. -1 1]  bottom-wall-sphere-bottom-back)));;back right area.  Connects to main case
                        (bottom-hull   (thumb-place 3/2 thumb-back-y (translate [0 -1 1]  bottom-wall-sphere-bottom-back)))
						   )

                          (hull;;left
                         (bottom-hull  (thumb-place (+ 5/2 0.05) thumb-back-y (translate [1 -1 3]  bottom-wall-sphere-bottom-back)))
                       (bottom-hull    (thumb-place 3/2 thumb-back-y (translate [0 -1 1]  bottom-wall-sphere-bottom-back)))
						 )
                          #_ (hull
                         (bottom-hull (thumb-place (+ 5/2 0.05) thumb-back-y (translate [1 -1 1]  bottom-wall-sphere-bottom-back)))
						   )
                          (hull;;Removed this for better fit
                         (bottom-hull  (thumb-place 0.5 thumb-back-y (translate [0. -1 1]  bottom-wall-sphere-bottom-back)))
						   
                         (bottom-hull  (case-place left-wall-column 1.6666 (translate [0.5 -1.5 1]  bottom-wall-sphere-bottom-front)));;not needed?
						   )
                          ])
		;;Replacement if above does not work out				  
 #_(def thumb-back-wall-lower [(hull
                        (bottom-hull   (thumb-place 0.55 thumb-back-y (translate [-0.2 -1 1]  bottom-wall-sphere-bottom-back)));;back right area.  Connects to main case
                        (bottom-hull   (thumb-place 3/2 thumb-back-y (translate [0 -1 1]  bottom-wall-sphere-bottom-back)))
						   )
                       ...
                       (bottom-hull    (thumb-place 3/2 thumb-back-y (translate [0 -1 1]  bottom-wall-sphere-bottom-back)))
                         ...
                         #_ (hull;;Removed this for better fit
...
(def  thumb-left-wall-lower [
						(hull ;;top section
                        (bottom-hull   (thumb-place thumb-left-wall-column thumb-back-y (translate [1 -1 3]  bottom-wall-sphere-bottom-back)));;New 1 -1 3;;old [2 -1 1]					   
...
;;;thumb cluster side (
	(hull
		(bottom-hull    (case-place 0.70 3.95 (translate [0 1 1]  bottom-wall-sphere-thumb-side)))
		(bottom-hull   (case-place 0.72 3.86 (translate [-2 -1 5]  bottom-wall-sphere-thumb-front)) )
	)
(hull
		(bottom-hull    (thumb-place thumb-right-wall thumb-front-row-sides (translate [-1 2.4 1]  bottom-wall-sphere-bottom-front)))
		(bottom-hull   (case-place 0.72 3.87 (translate [-2 -1 5]  bottom-wall-sphere-thumb-front)) )
...
(def right-wall-lower (concat
	 ...
	 [(union
	   ...
	   (hull (bottom-hull(case-place right-wall-column-bottom  4 (translate [-1 0 1] ( bottom-wall-sphere-bottom 1/2))))
			 (bottom-hull(case-place right-wall-column-bottom   4 (translate [0 1 1] ( bottom-wall-sphere-bottom 0))))
			 )
	...
	(let [place case-place]
			 [(hull  (bottom-hull(place left-wall-column -0.18 (translate [1 -1 1] bottom-wall-sphere-bottom-back)))
			  ...
			  (hull  (bottom-hull(place left-wall-column 2 (translate [1 0 1] bottom-wall-sphere-bottom-back)))
					 (bottom-hull(place left-wall-column 1.6666  (translate [0 -1.5 1] bottom-wall-sphere-bottom-front))));;;was 1 0 1
			#_  (hull  (bottom-hull(place left-wall-column 1.6666  (translate [1 0 1]  bottom-wall-sphere-bottom-front))))])
)			
(def left-wall-lower
			(if (== row-size 0) left-wall-dactyl
...
(def ergo_screw_y1 (if (== alpha_ergo_style 1) (+ row-size 1/5) (+ row-size 1/2) )) ;;;top mount position
(def ergo_screw_y2 (+ 3 8/10))


(def screw_x1 (if (== alpha_ergo_style 1) (+ 4 7/10) (if (== ncols 5) 4 (+ 4 5/10))))
(def screw_y1  (if (== alpha_ergo_style 0)  (+ row-size 1/2) (+ row-size 1/5)))
(def screw_y2 (if (== alpha_ergo_style 0)  (+ 3 1/2) (+ 3 8/10)))

(def screw_x3 1/2)		;;default 0.5 --top left mount x position
(def screw_y3 (+ row-size 0.5))		;;default 1.5 ---top left mount y position---try .6

(def screw_y31 0.5)		;;default 1.5 ---top left mount y position---try .6

(def screw-hole (->> (cylinder 1.6 20)
                     (translate [0 0 3/2])
                     (with-fn wall-sphere-n)))	
...
(def pro-micro-position (if (>= 1 row-size) [-3.5 (- 37 (* row-size 20)) 4] [-3.5 34 4]))		;(def pro-micro-position [-18 16 4])
...
(def trrs-holder-position  (if (>= 1 row-size) [-21.5 (- 47. (* row-size 20)) 5] [-21.5 46 4]))
(def trrs-holder-size [12 13.6 9.2 ])
(def trrs-hole [10.7 12.7 11])
(def trrs-hole-front [7. 8. 8. ])
(def trrs-hole-back [3.35 12])
...
(def plate-cutout1
       (difference  (->>  (union  (for [xyz (range 1 12 1.7)];controls the scale last number needs to be lower for thinner walls
                    (->> (union
						...
						(translate [(* -1 xyz) xyz 0] left-wall-lightcycle)
						(translate [1 xyz 0] right-wall-lower)
					;	 left-wall-lower
					  ;(translate [0 0 -3])
					)) 				
                )
		...
		(mirror [0 -1 0]
			(translate [-47.4 22 3.7] 
				(scale [1 1 2] (union
					...
					(translate [0 0 0](text '"Liberation Sans" 15 case-text))
					(translate [-13 15 0](text '"Liberation Sans" 15 case-text2))
...
(difference
	(cube 13 9 7.5)
	(->> (cube 7.8 4.7 7) (translate [0 0 1.]))
	(->> (cube 1.2 3.5 4) (translate [3.3 -3 2]))
	(->> (cube 1.2 3.5 4) (translate [-3.3 -3 2]))
	(->> (cylinder 2 5)(with-fn 200) (rotate  (/  π 2)  [1 0 0])(translate [0 4 1.25]))
	
)
)
(def reset_button_hole
(->> (cylinder 2 5)(with-fn 200) (rotate  (/  π 2)  [1 0 0])(translate [0 4 1.25]))
)
(def bottom-case
	(scale[(- scale-bottom 0.0), scale-bottom, scale-bottom] 
	   (difference
			(union				
					...
					(difference 
							(union 						
							(->> reset_button (translate [13.5 (- 73.25 (* row-size 20)) 4.5]))
							(->> (translate [0 -16 0]usb-holder))
							(->> (translate [0 -18 0]trrs-holder))
			...
			(->> reset_button_hole (translate [13.5 (- 73 (* row-size 20)) 4.5]))		
			(if (== row-size 2) (->> (translate [0 -16 0] usb-holder-hole)		;;changed
			(->>(translate [0 -1. 0] trrs-holder-hole))))
		...
		(if (== wrist-rest-on 1) (->> rest-case-cuts	(translate [20 -93.5 0])	);;add/remove the wrist rest holes
  ...
  (difference
	   (union key-holes
		...
		(->> bottom-sides
			(translate [0 0 -1.6]))
	   screw-holes
	 #_  (->> (cube 150 250 300)(translate [-45 0 100]))
	)
